...
var theme = {
  ...
  */
   ...
   * Also handles dropdowns within the offcanvas
   */
  offCanvas: () => {
    var navbar = document.querySelector(".navbar");
    if (navbar == null) return;
    ...
    // Initialize dropdowns in offcanvas
    const offcanvasDropdowns = document.querySelectorAll('.offcanvas-nav .dropdown-toggle');
    offcanvasDropdowns.forEach(dropdown => {
      dropdown.addEventListener('click', function(e) {
        ...
        if (parentDropdown.classList.contains('show')) {
          // Close this dropdown and all children dropdowns
          closeDropdownTree(parentDropdown);
        } else {
          // Close sibling dropdowns at the same level
          const siblingDropdowns = getSiblingDropdowns(parentDropdown);
    ...
    // Helper function to close a dropdown and all its children dropdowns
    function closeDropdownTree(dropdown) {
      ...
      // Find and close all nested dropdowns
    ...
    // Helper function to get all sibling dropdowns
    function getSiblingDropdowns(dropdown) {
      ...
      if (parent) {
        ...
        for (let i = 0; i < children.length; i++) {
          if (children[i] !== dropdown && children[i].classList.contains('dropdown')) {
    ...
    document.addEventListener('click', function(e) {
      if (!e.target.closest('.offcanvas-nav .dropdown')) {
        const openDropdowns = document.querySelectorAll('.offcanvas-nav .dropdown.show');
    ...
    // Close dropdowns when offcanvas closes
   ...
   * Desktop: click navigates to the parent URL.
   * Mobile: click keeps dropdown toggle behavior.
   */
  parentDropdownLinks: () => {
    const selector = '[data-nav-parent-link="true"]';

    function syncParentDropdownLinks() {
